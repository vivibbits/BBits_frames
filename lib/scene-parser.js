...
function parseProjectDir(projectDir) {
  ...
  const editableElements = {}; // sceneNum -> [elementKey]
  ...
  for (const file of sceneFiles) {
    ...
    // Extract editable elements automatically from getBbitsKeyframeOverride calls
    const EDITABLE_ELEMENT_RE = /(?:window\.)?getBbitsKeyframeOverride\(\s*(\d+)\s*,\s*['"]([^'"]+)['"]/g;
    EDITABLE_ELEMENT_RE.lastIndex = 0;
    let em;
    while ((em = EDITABLE_ELEMENT_RE.exec(src)) !== null) {
      const sNum = parseInt(em[1]);
      const elKey = em[2];
      if (!editableElements[sNum]) editableElements[sNum] = [];
      if (!editableElements[sNum].includes(elKey)) {
        editableElements[sNum].push(elKey);
      }
    }
  }

  scenes.sort((a, b) => a.num - b.num);
  return { scenes, assets: Object.fromEntries(assets), editableElements };
}
